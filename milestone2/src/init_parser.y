...
%type<Node> bitwise_operators if_expression operand typedecl bitwise_operator file statements statement simple_stmts simple_stmt compound_stmt simple1 simple2 assignment return_stmt raise_stmt assert_stmt global_stmt nonlocal_stmt multi_targets_assgn augassign names block class_def function_def is_arguments is_fn_expression params param_nd param_wd  param_no_default param_with_default param annotation param_default if_stmt elif_stmt else_block while_stmt for_stmt expressions expressions_comma expression disjunction conjunction inversion comparison  bitwise_xor bitwise_or bitwise_and shift_expr sum term factor power primary atom group string strings list arguments kwargs kwarg
...
operand: NAME DLM_LFT_PRN DLM_RGT_PRN  
    ...
    | NAME DLM_LFT_SQ expressions DLM_RGT_SQ 
...
void log_reduce(const int rule_line, vector<string> tokens)
{
	if (verbose)
	{
		printf("Reduce: Rule at Line: %d, Token Reduced: %s", rule_line, tokens[0].c_str());
		cout << endl;
		...
		for(int i = 0; i < verbose_stack.size(); i++)
			printf("| %s ", verbose_stack[i].c_str());
		printf("|\n\n\n");
		cout << endl;
	}
}