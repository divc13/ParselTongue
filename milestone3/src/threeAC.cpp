		...
		if (children[0] -> name == "l_primary" && children.size() == 3)
		{
			...
			else
			{
				if (operation == "**")
				{
					...
					string label5 = newLabel();
					inst.field_1 = "if";
					inst.field_2 = tmp2;
					inst.field_3 = "goto";
					inst.field_4 = label5;			// fill
					inst.field_5 = "";
					inst.label = label6;
					threeAC.push_back(inst);
	...
	if (type != "IDENTIFIER" && name == "power")
	{
		...
		*/
		...
		if (children.size() == 3)
		{
			...
			filler.push_back({label6, label7});
